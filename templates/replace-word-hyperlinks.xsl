...
<xsl:stylesheet xmlns:rs="http://schemas.openxmlformats.org/package/2006/relationships" xmlns:xd="http://www.oxygenxml.com/ns/doc/xsl" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships" xmlns:w="http://schemas.openxmlformats.org/wordprocessingml/2006/main" xmlns:doctorx="http://gspring.com.au/pekoe/templates/docxt" xmlns:xs="http://www.w3.org/2001/XMLSchema" exclude-result-prefixes="xs xd doctorx rs" version="2.0">
    <xsl:param name="links-doc"/>
    <xsl:variable name="links" select="doc($links-doc)//rs:Relationship"/><!-- Replace the word hyperlinks with an anchor to make it simple.
        Probably TOO SIMPLE - I'm losing the Style.
        The important action starts when it encounters an element containing the HYPERLINK instruction.  --><!--  convert instrText content into a useable placeholder name. (Strip whitespace and quotes - I haven't done the quotes) -->
    <xsl:function name="doctorx:fix-field-name"><!-- instrText:
    ...
    </xsl:function><!-- the main action. Replace a Complex Field Character structure with a fldSimple.
    -->
    <xsl:template match="*[w:r/w:instrText[contains(.,'http://pekoe.io')]]"><!-- this matches a para, tc or similar container. Copy it, and its attributes, then process the first child. 
    ...
    <xsl:template mode="outside" match="node() | @*"><!-- not sure if attributes need to be matched here. --><!-- Initially, this should match a Run (r). if the run contains a complex-field-character "begin", switch modes. else copy the content -->
        <xsl:choose>
            <xsl:when test="descendant::w:fldChar/@w:fldCharType[. eq 'begin']"><!-- Don't copy - just change modes and go next. -->
    ...
    <xsl:template mode="inside" match="node()"><!-- These nodes are following a "begin". -->
        <xsl:choose>
            <xsl:when test="descendant::w:fldChar/@w:fldCharType[. eq 'end']"><!-- Don't copy it. Switch back to outside for the following sibling -->
                <xsl:apply-templates mode="outside" select="following-sibling::*[1]"/>
            </xsl:when>
            <xsl:when test="descendant::w:instrText[contains(.,'HYPERLINK')]"><!-- Sadly, this is NOT the Run I need to copy. The one I want is after the "separator" - the second following-run
    ...
    </xsl:template><!-- Delete the hyperlink style <w:rStyle w:val="Hyperlink"/> -->
